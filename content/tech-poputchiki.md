...
[docs.poputchiki.apiary.io](http://docs.poputchiki.apiary.io/)

URL указаны относительно http://poputchiki.ru
...
Такие вычисления являются очень требовательными к ресурсам, поэтому выполняются не в API, а делегируются. 
Сервер конвертации обычно запущен в нескольких экземплярах, что позволяет выполнять балансировку 
нагрузки и мастштабировать сервис, также он может быть физически
...
[![Схема](/poputchiki.svg)](/poputchiki.svg)

Упрощенная схема инфраструктуры. Увеличение по клику.

Специально для проекта была разработана инфраструктура, основанная на операционной системе CoreOS.
...
Эта технология позволяет избавиться от разрешения конфликтов зависимостей и максимально приближает условия тестирования к боевым, т.е. практически устраняет различия между тем, как проект выполняется на тестовом, отладочном и боевом серверах.
...
(например, сервер конвертации требует redis), запуск, перезапуск, обновление и повторный запуск при сбое контейнера, переонс контейнеров при физическом выходе из строя сервера, позволяет производить настройку автоматически (ip адреса контейнеров с MongoDB, Redis можно с легкостью получать через единый интерфейс). 
...
#### Flannel
Flannel — проект от CoreOS, значительно упрощающий сетевое взаимодествие между контейнерами. Каждому контейнеру присваивается уникальный
в пределах кластера ip, по которому с ним может связываться и взаимодействовать другой контейнер из кластера. Благодаря этому модулю, зная лишь ip адрес контейнера, можно с ним соединиться, независимо от того, на каком физическом сервере он находится, не прибегая к пробросу портов и сложной настройке сети.

Для использования CoreOS и Flannel был разработан автоматический установщик, выполняющий первичную настройку Docker и установку Flannel.
...
## Проекты, разработанные для поддержки инфраструктуры и дополнительные библиотеки
...
Веб интерфейс: [cydev.ru/webhook](https://cydev.ru/webhook)

Исходный код: [ernado/cyhooks](https://github.com/ernado/cyhooks)
...
Исходный код: [ernado/gomp](https://github.com/ernado/gomp)


### Gorobokassa
Библиотека для использования Робокассы, написана на Go.

Исходный код: [ernado/gorobokassa](https://github.com/ernado/gorobokassa)


### Библиотеки авторизации через социальные сети на Go
Одноклассники: [ernado/gookauth](https://github.com/ernado/gookauth)

Вконтакте: [ernado/govkauth](https://github.com/ernado/govkauth)

Facebook: [ernado/gofbauth](https://github.com/ernado/gofbauth)

### gosmsru
Библиотека для отправки sms через сервис sms.ru

[ernado/gosmsru](https://github.com/ernado/gosmsru)


### Revealer
Небольшая программа, упрощающая процесс обнаружения сервисов для контейнеров с использованием Docker и etcd.

[ernado/revealer](https://github.com/ernado/revealer)

### Docker образы Nginx
#### [cydev/nginx](https://github.com/cydev/nginx)
Базовый образ, с последней версией nginx, поддержкой spdy и pagespeed

#### [cydev/nginx-balancer](https://github.com/cydev/nginx-balancer)
Образ балансировщика переднего края 

#### [cydev/nginx-backend](https://github.com/cydev/nginx-backend)
Базовый образ балансировщика для API



### Docker образы
#### cydev/mongo
Базовый образ для базы данных MongoDB